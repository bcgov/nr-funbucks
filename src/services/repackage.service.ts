...
export class RepackageService {
  ...
  public async package(serverConfig: ServerConfig): Promise<void> {
    const files = (await this.getFiles(OUTPUT_BASEPATH)).map((file) => ({
      file,
  ...
  public generateVolume(serverConfig: ServerConfig, outpaths: Outpath[]) {
    const indent = serverConfig.oc?.volume.indent
      ? serverConfig.oc?.volume.indent
      : 0;
  ...
  public generateConfigMap(serverConfig: ServerConfig, outpaths: Outpath[]) {
    ...
    const prefix = serverConfig.oc?.configmap.prefix
      ? serverConfig.oc?.configmap.prefix
      : '';
    const suffix = serverConfig.oc?.configmap.suffix
      ? serverConfig.oc?.configmap.suffix
      : '';
